...
class MANO_Renderer: #Abstract Base Class. Don't use this!
    ...
    def render_hand(self, pose, shape, tex_param, angle=0, axis="Z", is_dorsal=True):
        ...
        # yy: new -> rotation
        images_ori, image_cubes = render_img_rotate(verts, faces_idx, faces_uvs, verts_uvs, new_tex_img, jts, angle, axis)
        images_edited_texture, _ = render_img_rotate(verts, faces_idx, faces_uvs, verts_uvs, new_tex_img_edited, jts,
                                                    angle, axis)
        ...
        return images_ori, images_edited_texture, image_cubes, vertices, new_tex_img, joints / 1000
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        cv2.imwrite("final_imgs/ventral/rendered_hand_{}.png".format(str(i).zfill(3)),
                    (images_ori[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("final_imgs/ventral/edited_texture_hand_{}.png".format(str(i).zfill(3)),
                    (images_edited_texture[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("final_imgs/ventral/texture_{}.png".format(str(i).zfill(3)),
                    (new_tex_img[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("final_imgs/ventral/cubes_{}.png".format(str(i).zfill(3)),
                    (image_cubes[0, ..., :3].cpu().detach().numpy() * 255).astype(int)[..., ::-1])
        np.save("final_imgs/ventral/kp_{}.npy".format(str(i).zfill(3)), np.squeeze(joints))
    ...
    for i in range(10):
        ...
        axis = "X"
        images_ori, images_edited_texture, image_cubes, vertices, new_tex_img, joints = renderer.render_hand(pose_param, shape_param, tex_param, angle, axis)
        cv2.imwrite("final_imgs/dorsal/rendered_hand_{}.png".format(str(i).zfill(3)),
                    (images_ori[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("final_imgs/dorsal/edited_texture_hand_{}.png".format(str(i).zfill(3)),
                    (images_edited_texture[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("final_imgs/dorsal/texture_{}.png".format(str(i).zfill(3)),
                    (new_tex_img[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("final_imgs/dorsal/cubes_{}.png".format(str(i).zfill(3)),
                    (image_cubes[0, ..., :3].cpu().detach().numpy() * 255).astype(int)[..., ::-1])
        np.save("final_imgs/dorsal/kp_{}.npy".format(str(i).zfill(3)), np.squeeze(joints))
