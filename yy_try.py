...
def render_img_rotate(verts, faces_idx, faces_uvs, verts_uvs, new_tex_img, kp, angle, axis, is_bicolor=False):
    # Set environment
    ...
    # yy: rotate light by the mesh
    # rot = RotateAxisAngle(angle, axis)
    # init_light_location = [[0.0, 0.0, -3.0]]
    # rot_loc_light = rot.transform_points(torch.tensor(init_light_location)).numpy()

    # yy: fix light location
    rot_loc_light = [[0.0, 2.0, 0.0]]

    if is_bicolor:
        lights = PointLights(device=device, location=rot_loc_light, specular_color=((0.0, 0.0, 0.0),),
                             diffuse_color=((0.0, 0.0, 0.0),), ambient_color=((1, 1, 1),))
    else:
        lights = PointLights(device=device, location=rot_loc_light, specular_color=((0.0, 0.0, 0.0),), diffuse_color=((0.7, 0.7, 0.7),))
    ...
    # yy: rotate keypoints
    rot = RotateAxisAngle(angle, axis)
    rot_kp = rot.transform_points(torch.squeeze(kp))




    return images, image_cubes, rot_kp



# yy: function used by Render_HTML.py, used for rendering cubes
def render_cubes(verts, faces_idx, faces_uvs, verts_uvs, new_tex_img, kp, angle, axis, is_bicolor=False):
    # Set environment
    ...
    if is_bicolor:
        ...
        lights = PointLights(device=device, location=rot_loc_light, ambient_color=((1, 1, 1),), specular_color=((0.0, 0.0, 0.0),), diffuse_color=((0, 0, 0),))
...
def create_cubes_and_relocate(kp, size_ratio=0.0006):  # ori: 0.0003
    """
    ...
    for i in range(21):
        ...
        # increasing color from 0.0 - 1.0, evenly distributed. duplicated 8 times for each cube
        color = torch.tensor([(i * 0.96) / 21] * 8).unsqueeze(1).repeat(1, 3)
        colors = torch.cat((colors, color), 0)
...
# for y in [90]:
#     for z in [90]:
#         for l_x in range(-num, num):
#             for l_y in range(-num, num):
#                 for l_z in range(-num, num):
#                     cnt += 1
#                     print(l_x, l_y, l_z, cnt)
#                     l_x, l_y, l_z = 0, 5, 0
#
#
#
#                     light_position = [[l_x, l_y, l_z]]
#
#                     # Setup
...
#                     # -z direction.
#                     # yy: tune light here
#                     lights = PointLights(device=device, location=light_position, specular_color=((0.0, 0.0, 0.0),), diffuse_color=((0.7, 0.7, 0.7),))
#
...
#                     plt.imshow(images[0, ..., :3].cpu().numpy())
#                     plt.axis("off")
#                     plt.savefig('test2/t{}.png'.format(cnt))
#                     a=1