...
from yy_try import render_img, render_img_rotate
...
class MANO_Renderer: #Abstract Base Class. Don't use this!
    ...
    def render_hand(self, pose, shape, tex_param, angle=0, axis="Z", is_dorsal=True):
        ...
        # yy: to be deleted
        new_tex_img_edited = cv2.imread('./edited_texture.png')
        new_tex_img_edited = TF.to_tensor(new_tex_img_edited)
        new_tex_img_edited.unsqueeze_(0)
        new_tex_img = torch.transpose(torch.transpose(new_tex_img_edited, 1, 3), 1, 2)

        new_tex_img_edited = cv2.imread('./thumbnail_blur_edited_texture.png')
        new_tex_img_edited = TF.to_tensor(new_tex_img_edited)
        new_tex_img_edited.unsqueeze_(0)
        new_tex_img_ori = torch.transpose(torch.transpose(new_tex_img_edited, 1, 3), 1, 2)

        # yy: Load edited texture
        # new_tex_img_edited = cv2.imread('./edited_texture.png')
        # new_tex_img_edited = cv2.imread("./thumbnail_blur_edited_texture.png")
        new_tex_img_edited = cv2.imread("./blur_edited_texture_v2.png")
        new_tex_img_edited = TF.to_tensor(new_tex_img_edited)
        new_tex_img_edited.unsqueeze_(0)
        ...
        # yy: new -> rotation
        images_ori, image_cubes = render_img_rotate(verts, faces_idx, faces_uvs, verts_uvs, new_tex_img_ori, joints / 1000,
                                                    angle, axis)
        images_ori_ori, image_cubes = render_img_rotate(verts, faces_idx, faces_uvs, verts_uvs, new_tex_img, joints / 1000, angle, axis)
        images_edited_texture, _ = render_img_rotate(verts, faces_idx, faces_uvs, verts_uvs, new_tex_img_edited, joints / 1000,
                                                    angle, axis)
        ...
        return (images_ori_ori, images_ori), images_edited_texture, image_cubes, vertices, new_tex_img, joints / 1000
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        print(angle)
        axis = "X"
        images_ori_, images_edited_texture, image_cubes, vertices, new_tex_img, joints = renderer.render_hand(pose_param, shape_param, tex_param, angle, axis)
        ori_ori, images_ori = images_ori_
        cv2.imwrite("{}/ventral/ori_hand_{}.png".format(folder_name, str(i).zfill(3)),
                    (ori_ori[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/ventral/rendered_hand_{}.png".format(folder_name, str(i).zfill(3)),
                    (images_ori[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/ventral/edited_texture_hand_{}.png".format(folder_name, str(i).zfill(3)),
                    (images_edited_texture[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/ventral/texture_{}.png".format(folder_name, str(i).zfill(3)),
                    (new_tex_img[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/ventral/cubes_{}.png".format(folder_name, str(i).zfill(3)),
                    (image_cubes[0, ..., :3].cpu().detach().numpy() * 255).astype(int)[..., ::-1])
        np.save("{}/ventral/kp_{}.npy".format(folder_name, str(i).zfill(3)), np.squeeze(joints))
    ...
    for i in range(5):
        ...
        images_ori_, images_edited_texture, image_cubes, vertices, new_tex_img, joints = renderer.render_hand(pose_param, shape_param, tex_param, angle, axis)
        ori_ori, images_ori = images_ori_
        cv2.imwrite("{}/dorsal/ori_hand_{}.png".format(folder_name, str(i).zfill(3)),
                    (ori_ori[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/dorsal/rendered_hand_{}.png".format(folder_name, str(i).zfill(3)),
                    (images_ori[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/dorsal/edited_texture_hand_{}.png".format(folder_name, str(i).zfill(3)),
                    (images_edited_texture[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/dorsal/texture_{}.png".format(folder_name, str(i).zfill(3)),
                    (new_tex_img[0, ..., :3].cpu().numpy() * 255).astype(int)[..., ::-1])
        cv2.imwrite("{}/dorsal/cubes_{}.png".format(folder_name, str(i).zfill(3)),
                    (image_cubes[0, ..., :3].cpu().detach().numpy() * 255).astype(int)[..., ::-1])
        np.save("{}/dorsal/kp_{}.npy".format(folder_name, str(i).zfill(3)), np.squeeze(joints))
